...
public:
	...
	DEFINE_MEMBER_FN(ctor, BSShaderTextureSet *, 0x012DFFB0);
};
...
public:
	...
	DEFINE_MEMBER_FN(GetSex, char, 0x00351420);
	DEFINE_MEMBER_FN(HasOverlays, bool, 0x00368BA0);

	// Swaps a headPart of the same type as target with target
	DEFINE_MEMBER_FN(ChangeHeadPart, void, 0x00365D70, BGSHeadPart * target);
	...
	DEFINE_MEMBER_FN(UpdateNeck, void, 0x00360850, BSFaceGenNiNode * faceNode);

	// Computes RGB SkinTone from RGBA TintMask
	DEFINE_MEMBER_FN(SetSkinFromTint, void, 0x00360600, NiColorA * result, TintMask * tintMask, UInt32 compute, UInt32 unk1);
