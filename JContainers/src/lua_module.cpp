...
namespace lua { namespace aux_wip {
	...
	private:
		...
		static const char* top_string(lua_State *l) {
			...
			return "<can't obtain error-string>";
		}
	...
	class context_pool : collections::dependent_context, boost::noncopyable {
		// will be not enough if 129 threads consume and then release lua-contexts
		static const uint32_t queue_capacity = 128;
	...
	TEST_F(fixture, Lua, trtr)
	{
		...
		auto task = [&]() {
			while (stop.load(std::memory_order_acquire) == 0) {
				EXPECT_TRUE(autofreed_context(pool).context().eval_lua_function(nullptr, "return testing.perform()")->intValue() != 0);
