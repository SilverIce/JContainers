...
This feature simplifies access to nested information. There is a group of `solve*` and `solve*Setter` functions. `solve*` retrieves values and `solve*Setter` changes (assigns) values. Suppose there is object info containing the following information:
```json
{
    "classicPreset" :  {
        "campfireLighting" : "Automatic"
    },
    "numbers" : [0, 1, 2, 3]
}
```
Accessing the campfireLighting string usually looks like:
```lua
int preset = JMap.getObj(info, "classicPreset")
string lightingType = JMap.getStr(preset, "campfileLighting")
```
But there's a simpler way to retrieve or change the campfireLighting value or access the numbers array:
```lua
string lightingType = JValue.solveStr(info, ".classicPreset.campfireLighting")
JValue.solveStrSetter(info, ".classicPreset.campfireLighting", "Non-Automatic")
...
You can manually change the status of an object's lifetime management:
- To prevent destruction you must own the container (use `JValue.retain` function)
...
The lifetime model is implemented using simple owner(reference) counting. Each object has a counter. Each time the object gets inserted into another container or `JValue.retain` is called, the counter increases by 1. Each time the object gets removed from a container or released via `JValue.release`, the reference counter decreases by 1.
If the reference counter reaches zero, the object is temporarily owned for roughly 10 seconds. During this perod it has a 'last chance to survive' and gets destroyed if nobody owns it.

Newly created objects (Object created with `object`, `objectWith*`, `all/Keys/Values` or `readFromFile`) also have that 'last chance to survive'.

Retain/release illustration:
